...
export default function Header({ variant = 'sticky' }: HeaderProps) {
  ...
  const [isLeadFormOpen, setIsLeadFormOpen] = useState(false); // State for LeadFormModal
  ...
  return (
    <>
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`transition font-medium ${isActive(link.href)
                    ? 'text-accent-700'
      ...
      <LeadFormModal
        ...
        initialRoomName="" // No specific room selected from header
      />
    </>
  );
}
